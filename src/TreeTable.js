...
const ExpandButtonCons = (props) => {
    return (
      <button
        {...props}>
          {props.expanded === null ? <span>&nbsp;</span> : props.expanded ? '-' : '+'}
      </button>
    )
}
...
export default class TreeTable extends Component {
    ...
    mapLine = (line) => {
        const expandButton = <ExpandButtonCons 
    ...
    render() {
        const buttonExpand = <ExpandButtonCons expanded={this.expandDefault} onClick={this.expandAll}/>
        const expandHead = {
        ...
        <table style={this.props.style}>
            <thead>
                {!headCols ? null : <TrHead>{headCols}</TrHead>}
            </thead>
            <tbody>
                <Trs
                  {...this.props}
